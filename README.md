...
実用性はありません。開くと20万人ほどの行列の最後尾にいて、前の人たちの背中がずっと奥まで続いています。
...
- **人の列**: 画面のほとんどは前に並んでいる人たちで埋まっています（`QueueCrowdView`）。
  奥に向かって小さくなり、空気に溶けて薄くなります。人は等間隔に置いてあるので、
  1人ぶん進みきった瞬間に全体をひとつ手前へずらしても絵は変わらず、途切れずに前進して見えます。
- **前の人を叩く**: 画面をタップすると前の人を叩けます。振り返る、ため息をつく、
  やがて反応しなくなる、と回数に応じて態度が悪くなります。2%の確率で相手が列を抜け、1人ぶん進みます。
- **列の進行**: 位置は保存せず、基準時刻からの経過時間だけで毎回計算します（`QueueEngine`）。
  昼はおよそ4秒に1人、深夜でも10秒に1人は進みます。同じ入力には必ず同じ結果を返すため、
